...
export const kycValidationSchema: ValidationSchema = {
  ...
  dob: validate.custom((value: any) => {
    ...
    const isBirthdayPassed = m > 0 || (m === 0 && d >= 0); // If birth date has passed this year

    const finalAge = isBirthdayPassed ? age : age - 1;

    if (finalAge < 18) return "You must be at least 18 years old";
    if (finalAge > 120) return "Please enter a valid birth date";
...
export const validateKycForm = (data: KycFormData) => {
  return validateForm(data, kycValidationSchema);
};
