...
jest.mock("../SignupModal", () => ({
  SignupModal: ({ isOpen }: { isOpen: boolean }) => {
    return isOpen ? <div data-testid="signup-modal">Mock Signup Modal</div> : null;
  },
}));
...
describe("Header", () => {
  it("renders the left and center logos", () => {
    ...
    ).toBeInTheDocument(); // Left logo

    expect(
      screen.getAllByAltText("logo")[1]
    ).toBeInTheDocument(); // Center logo
  });
