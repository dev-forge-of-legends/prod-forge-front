...
interface TeamMatchInfoTeamItemProps {
  key: any;
  avatar: string;
  teamName: string;
  members: number;
...
const TeamMatchInfoTeamItem: React.FC<TeamMatchInfoTeamItemProps> = (props) => {
    ...
    <div className="flex flex-row justify-between items-center border-1 border-white/10 bg-black/30 rounded-lg p-2 md:p-3 shadow-[inset_4px_4px_20px_0_rgba(100,55,27,0.4)] min-w-65 w-full">
      <div className="flex flex-row items-center gap-2">
        ...
        <div className="flex flex-col justify-between text-white w-full">
          <p className="font-vastagoMedium text-[16px] md:text-[18px]">{props.teamName}</p>
          <p className="font-vastagoMedium text-gray-400 text-[12px] md:text-[14px]">{props.members} members</p>
        </div>
      </div>
...
export default TeamMatchInfoTeamItem;